...
namespace cookiejar
{
	...
	{
	public:
		inline SpriteDraw(Sprite *sprite, const Vector2 &offset = { 0, 0 }, const int &depth = 0, const BasePrecision &fps = 30) : Draw(sprite_draw_behaviour, depth),
			sprite(sprite),
			offset(offset),
			fps(fps),
			accumulator(0.0),
